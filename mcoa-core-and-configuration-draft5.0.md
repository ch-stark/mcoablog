...
**ACM 5.0 re-enables the Endpoint Collector in MCOA.** Early MCOA removed the spoke `observability-endpoint-operator`. That left no in-cluster process to point OpenShift Cluster Monitoring Operator (CMO) at the hub Alertmanager. ACM 5.0 puts the Endpoint Collector back on every MCOA managed cluster. It is the same `observability-endpoint-operator` image, started with the dedicated `mcoa` entrypoint ([ACM-34198](https://issues.redhat.com/browse/ACM-34198)). That process is **not** the old metrics collector: it starts only a CMO ConfigMap controller, injects the hub Alertmanager URL into `cluster-monitoring-config`, stays small, and does not federate metrics. [ACM-34602](https://issues.redhat.com/browse/ACM-34602) deploys it from MCOA. Collection stays on the Prometheus Agent.
...
| :--- | :--- | :--- |
| Spoke metrics collector | Custom metrics collector, deployed by the Endpoint Collector | Prometheus Agent, reconciled by Prometheus Operator |
| Endpoint Collector | Full endpoint operator (collector + CMO wiring) | **Re-enabled** in MCOA: `mcoa` entrypoint only. CMO `cluster-monitoring-config` and user-workload Alertmanager. Not the old collector |
...
## The Endpoint Collector is re-enabled in MCOA ([ACM-34198](https://issues.redhat.com/browse/ACM-34198))

The Endpoint Collector **is back** in ACM 5.0 MCOA. It was off in early MCOA; it is on again for every managed cluster that has platform metrics enabled.

MCOA still needs spoke Prometheus to **forward alerts** to the hub Alertmanager. OpenShift Cluster Monitoring Operator (CMO) reads that from the `cluster-monitoring-config` ConfigMap. There is not yet a CMO CRD you can apply from the hub as a `ManifestWork`, so ACM 5.0 keeps an in-cluster controller as a **bridge**. That controller **is** the Endpoint Collector.

The Endpoint Collector is the endpoint operator started with the `mcoa` subcommand ([ACM-34198](https://issues.redhat.com/browse/ACM-34198); implemented in [multicluster-observability-operator#2487](https://github.com/stolostron/multicluster-observability-operator/pull/2487)):
...
When OpenShift ships a declarative CMO CRD, this in-cluster mutator can go away: the hub will apply the config through `ManifestWork`. Until then, treat the re-enabled Endpoint Collector as part of ACM 5.0 MCOA, not as a return of the legacy metrics collector.

On a managed cluster:

```bash
# Endpoint Collector re-enabled in MCOA mode (not the legacy metrics collector)
...
4. MCOA **re-enables the Endpoint Collector**: it deploys the endpoint operator with the `mcoa` entrypoint ([ACM-34198](https://issues.redhat.com/browse/ACM-34198)). That is the Alertmanager bridge described above, not the old metrics collector.
...
3. On a managed cluster, confirm the Endpoint Collector is running (`observability-endpoint-operator` in `mcoa` mode) and `cluster-monitoring-config` lists the hub Alertmanager ([ACM-34198](https://issues.redhat.com/browse/ACM-34198)).
...
- [ACM-34198](https://issues.redhat.com/browse/ACM-34198) Endpoint Collector re-enabled in MCOA: dedicated `mcoa` entrypoint (CMO Alertmanager bridge)
- [ACM-34602](https://issues.redhat.com/browse/ACM-34602) Deploy the Endpoint Collector (endpoint operator) from MCOA
